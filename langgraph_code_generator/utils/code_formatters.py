...
def format_review_result(review_result: Dict) -> str:
    ...
    try:
        ...
        formatted.append(f"Overall Status: {'✅ Approved' if approved else '❌ Not Approved'}")
        
        # Process file-specific reviews
        files_elem = root.find("files")
        if files_elem is not None:
            for file_elem in files_elem.findall("file"):
                filename = file_elem.find("name").text
                formatted.append(f"\nFile: {filename}")
                formatted.append("-" * (len(filename) + 6))
                
                # Issues for this file
                issues = file_elem.find("issues")
                ...
                suggestions = file_elem.find("suggestions")
                ...
                comments = file_elem.find("comments")
        ...
        # If no file-specific reviews found, fall back to old format
        else:
